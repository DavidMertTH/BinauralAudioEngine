...
namespace Code
{
    public class AudioEnvironment : MonoBehaviour
    {
        ...
        public NativeArray<RaycastHit> RemoveDoubles(NativeArray<RaycastHit> rawHits)
        {
            ...
            NativeArray<RaycastHit> result = new NativeArray<RaycastHit>(uniqueHits.Length, Allocator.TempJob);
            NativeArray<RaycastHit>.Copy(uniqueHits.AsArray(), result);

            uniqueHits.Dispose(); // Jetzt sicher!

            return result; // Achtung: muss später auch disposed werden!
